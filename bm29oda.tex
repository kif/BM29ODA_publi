...
computer while the azimuthal integration pipeline, which needs the lowest
...
The sample changer allows a high throughput of different samples (\~3
mn/sample).
...
As the images are acquired with a pixel detector (Pilatus 1M from Dectris), the
...
\label{SM}
One dataset usually consists of data measured on the sample and
on the buffer. 
...
(prevents the interpretation of data from damaged samples).
