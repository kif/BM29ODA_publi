...
\title{Online data analysis at the ESRF bioSaxs beamline, BM29}
...
By varying the structural aspects of the function of proteins and other macromolecules can be studied at near physiological conditions.
...
As even the largest macromolecules due not elute from the SEC column immediately, the scattering from the buffer measured at the beginning of the experiment can be used for background correction.\\
...
At BM29 in SC mode this is currently 3 minutes each for 3 dilutions of sample with matching interspersed background measurements  (10 one second frames each, including both cleaning and loading).
...
As all the intrinsic assumptions required cannot be validated without additional information the experimentators ultimately bear the responsibility for the quality of their data and validity of their conclusions. 
...
If both values of $R_g$ are identical (differences are outside the Guinier range), or if \textit{autorg} fails for both subtracted curves, the buffer with the lowest total scattering is chosen, indicating this is the buffer scattering free from artifacts. 
Although contamination by spoiling typically affects the buffer after,  one could naively take the buffer before by default; however the the second scenario of bubble formation is equally likely for both buffers and makes this more complex approach necessary.
...
If the automated processing is not sufficient, the user can use the subtracted frames to take appropriate actions, such as  manual selection of a region to merge or use of advanced tools such as US-SOMO \cite{USSOMO} to deconvolute overlapping peaks.
